...
def test_fred_config_loads():
    ...
    assert cfg["version"].startswith("fred-series")
...
def test_regime_and_industry_scores_deterministic():
    latest = {
        "labor": {"series_id": "UNRATE", "date": "2026-07-01", "value": 4.1},
        "yield_curve": {"series_id": "T10Y2Y", "date": "2026-07-01", "value": 0.4},
        "inflation": {"series_id": "CPIAUCSL", "date": "2026-07-01", "value": 300.0},
        "policy_rate": {"series_id": "FEDFUNDS", "date": "2026-07-01", "value": 4.3},
        "industrial_production": {"series_id": "INDPRO", "date": "2026-07-01", "value": 103.0},
    }
...
def test_industry_qa_fail_blocks_low_overall(monkeypatch):
    # Direct function uses DB — test scoring gate logic via reasons path without DB:
    rules = load_industry_rules()
    assessment = {
